...
public class ProtocolLibHandler {
    ...
    public static void sendYawChange(Player player, Vector newDirection) {
        if (protocolManager == null)
            protocolManager = ProtocolLibrary.getProtocolManager();
        final PacketContainer yawPacket = protocolManager.createPacket(PacketType.Play.Server.LOOK_AT, false);
        if (enumArgumentAnchor_EYES == null) {
            if (ReflectionsUtil.isVersionHigherThan(1, 17)) {
                class_ArgumentAnchor = ReflectionsUtil.getClass("net.minecraft.commands.arguments.ArgumentAnchor$Anchor");
            } else {
        ...
        yawPacket.getModifier().write(4, enumArgumentAnchor_EYES);
        yawPacket.getDoubles().write(0, player.getEyeLocation().getX() + newDirection.getX());
        yawPacket.getDoubles().write(1, player.getEyeLocation().getY() + newDirection.getY());
        yawPacket.getDoubles().write(2, player.getEyeLocation().getZ() + newDirection.getZ());
